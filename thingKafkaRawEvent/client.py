...
def getConsumer(conf):
    ...
    consumer_conf['group.id'] = 'note_group_4'
...
def process(topics, record_value):
    record = json.loads(record_value)
    return json.dumps({
        'data': {
            'payload': record,
            'account_id': int(topics[1]),
            'serial_id': int(topics[3])
        }
    })
...
if __name__ == '__main__':
    ...
    producer_topic = 'kafka_raw_event'
    ...
    try:
        ...
        while count < 1:
            ...
            if msg is None:
                ...
                if topics[0] != 'app' or topics[-1] != 'raw_event':
                    continue
                try:
                    result = process(topics, record_value)
                    producer.produce(producer_topic, key=record_key, value=result, on_delivery=acked)
                    producer.poll(0)
